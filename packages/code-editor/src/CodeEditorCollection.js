...
export default class CodeEditorCollection extends PureComponent {
  ...
  constructor (props) {
    ...
    this.updateTabPath = this.updateTabPath.bind(this)
  }
  ...
  updateTabPath(replaceWithPublic) {
    const { currentTab, allTabs } = this.tabs.current
    const replacePath = (replaceWithPublic, oldTab) => {
      const { path } = oldTab
      const pathArr = path.split('/')
      pathArr[0] = replaceWithPublic ? 'public' : 'private'
      oldTab.path = oldTab.key = pathArr.join('/')
    }
    replacePath(replaceWithPublic, currentTab)
    allTabs.forEach(item => replacePath(replaceWithPublic, item))
    this.props.onSelectTab(currentTab)
    modelSessionManager.sessions = []
    this.setState({ selectedTab: { ...currentTab, path: currentTab.path } })
  }
